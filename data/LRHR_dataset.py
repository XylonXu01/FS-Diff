...
class LRHRDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.datatype == 'lmdb':
            ...
            img_HR = Image.open(self.hr_path[index]).convert("RGB") # 高分辨率
            img_SR = Image.open(self.sr_path[index]).convert("RGB") # 差值超分
            if self.need_LR:
                img_LR = Image.open(self.lr_path[index]).convert("RGB") # 低分辨率
        if self.need_LR:
            [img_LR, img_SR, img_HR] = Util.transform_augment( # 这个函数将图像列表转换为张量，并根据指定的分割方式对图像进行数据增强。
